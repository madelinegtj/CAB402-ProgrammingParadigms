...
let matrixMultiply (weights: WeightMatrix) (input: Vector) : Vector =
    ...
    let outputSize = weights.Length
    let output = Array.zeroCreate outputSize // Create mutable output array with size n

    // For each column of output element,
    for i = 0 to outputSize - 1 do
        let mutable sum = 0.0   // Temporary accumulator for the dot product
        let row = weights.[i]
        // For each row, accumulate the product of weight and input value
        for j = 0 to input.Length - 1 do
            sum <- sum + row.[j] * input.[j]
        
        // After calculating all rows, assign the accumulated sum to the output vector
        output.[i] <- sum
    output
